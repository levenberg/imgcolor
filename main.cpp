...
bool vec_comp(const vector<Point> &a, const vector<Point> &b){
    return a.size() > b.size();
}

void findNumber(Mat src){
    Mat gray,gray0,th;
    medianBlur(src,src,3);
    cvtColor(src, gray0, CV_BGR2GRAY);
    threshold(gray0,gray,100,255,CV_THRESH_BINARY_INV);
    //    Canny(th, gray, 5, 500, 3);
    vector<vector<Point> > contours;
    findContours(gray, contours, CV_RETR_LIST, CV_CHAIN_APPROX_NONE);
    int n=0;
    if (contours.size()>0){
        sort(contours.begin(),contours.end(),vec_comp);
        for (size_t i = 0; i < contours.size(); i++)
        {
            if (contours[i].size() < 50 || contours[i].size() > 2000)
            {
                continue;
            }
            Rect rect=boundingRect(contours[i]);
            if (rect.height*1.0/rect.width>1 && rect.height*1.0/rect.width<2)
            {
//                rectangle(src,rect, Scalar( 255, 255, 255 ), 3, 8 );
                cout<<rect.width<<"  "<<rect.height<<endl;
                Mat img_rect;
                src(rect).copyTo(img_rect);
                char str[100];
                sprintf(str,"%d.png",n++);
                imwrite(str,img_rect);
            ...
            //                       line(src,P[j],P[(j+1)%4],Scalar(255),3);
            //            }
...
int main(int argc,char* argv[])
{
    ...
    */
    ...
    while(1)
    {
        ...
        putText(img_clone,str1,Point(50,50),CV_FONT_HERSHEY_COMPLEX,0.5,Scalar(0,0,255),1);
        putText(img_clone,str2,Point(50,90),CV_FONT_HERSHEY_COMPLEX,0.5,Scalar(0,0,255),1);
        putText(img_clone,str3,Point(50,130),CV_FONT_HERSHEY_COMPLEX,0.5,Scalar(0,0,255),1);
        putText(img_clone,str4,Point(50,170),CV_FONT_HERSHEY_COMPLEX,0.5,Scalar(0,0,255),1);
